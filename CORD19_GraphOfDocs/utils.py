...
def list_filenames(dirpath):
    ...
    in the directory specified by dirpath.
    """
    return sorted([file for file in listdir(dirpath) if isfile(join(dirpath, file))])

def skip_filenames(lis, filename):
    """
    Function that returns a list of filenames
    not including the ones before the specified index.
    """
    return lis[lis.index(filename):]

def read_file(dirpath, file):
    """
    Function that reads a file in text mode, and returns its plain-text as a string,
    which has newline characters and non-printable characters removed.
